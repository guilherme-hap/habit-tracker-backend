...
router.post('/:id/complete', verifyToken, async (req, res) => {
  ...
  try {
    const existing = await HabitCompletion.findOne({
      where: { habitId, userId, date: isoDate }
    });

    if (existing) {
      await existing.destroy();
      return res.status(200).json({ message: 'Hábito desmarcado com sucesso' });
    } else {
      ...
      return res.status(201).json({ message: 'Hábito marcado como concluído', completion });
    }
  } catch (err) {
    console.error('Erro ao alternar conclusão do hábito:', err);
    return res.status(500).json({ error: 'Erro ao atualizar conclusão do hábito' });
  }
});
